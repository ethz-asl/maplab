...
class ViwlsGraph : public ::testing::Test {
 ...
 protected:
  ...
  virtual void SetUp() {
    ...
    addSemanticLandmarksAndMeasurements();
  ...
  void addSemanticLandmarksAndMeasurements();

  vi_map::VIMap vi_map_;
  pose_graph::VertexIdList vertex_ids_;
  vi_map::LandmarkIdList landmark_ids_;
  vi_map::SemanticLandmarkIdList semantic_landmark_ids_;
...
void ViwlsGraph::addVisualFrameToVertex(vi_map::Vertex* vertex_ptr) {
  ...
  frame->setSemanticObjectClassIds(object_class_ids);
...
void ViwlsGraph::addSemanticLandmarksAndMeasurements() {
  semantic_landmark_ids_.clear();

  CHECK(!vertex_ids_.empty());
  std::default_random_engine generator(kRandomSeed);
  std::uniform_int_distribution<int> distribution(0, vertex_ids_.size() - 1);

  for (unsigned int i = 0; i < kNumOfStoreSemanticLandmarks; ++i) {
    pose_graph::VertexId vertex_id_storing_landmark =
        vertex_ids_[distribution(generator)];

    vi_map::SemanticLandmarkId landmark_id;
    common::generateId(&landmark_id);
    semantic_landmark_ids_.push_back(landmark_id);

    vi_map::SemanticLandmark store_landmark;
    store_landmark.setId(landmark_id);
    // fills dummy class id
    store_landmark.setClassId(10);
    CHECK(!vertex_ids_.empty());
    vi_map_.getVertex(vertex_id_storing_landmark)
        .getSemanticLandmarks()
        .addSemanticLandmark(store_landmark);
    ...
    for (unsigned int j = 0;
      ...
      for (unsigned int k = 0; k < kNumOfVertices; ++k) {
        ...
        vi_map_.getVertex(vertex_id_storing_landmark)
            .getSemanticLandmarks()
            .getSemanticLandmark(landmark_id)
            .addObservation(
                vertex_ids_[k], kVisualFrameIndex,
                vi_map_.getVertex(vertex_ids_[k])
                        .observedSemanticLandmarkIdsSize(kVisualFrameIndex) -
                    1);
      }
    }
  }

  // Also add dummy measurement-uncertainty-descriptor data to vertices.
  for (unsigned int i = 0; i < kNumOfVertices; ++i) {
    vi_map::Vertex& vertex = vi_map_.getVertex(vertex_ids_[i]);
    const int num_of_measurements =
        vertex.observedSemanticLandmarkIdsSize(kVisualFrameIndex);

    Eigen::Matrix4Xd img_objects_distorted;
    Eigen::VectorXd object_uncertainties;
    aslam::VisualFrame::SemanticObjectDescriptorsT object_descriptors;
    Eigen::VectorXi object_class_ids;
    const unsigned int object_descriptor_size = 4096;
    img_objects_distorted.resize(Eigen::NoChange, num_of_measurements);
    object_uncertainties.resize(num_of_measurements);
    object_descriptors.resize(object_descriptor_size, num_of_measurements);
    object_class_ids.resize(num_of_measurements);
    img_objects_distorted.setZero();
    object_uncertainties.setZero();
    object_descriptors.setZero();
    object_class_ids.setZero();
    vertex.getVisualFrame(kVisualFrameIndex)
        .setSemanticObjectMeasurements(img_objects_distorted);
    vertex.getVisualFrame(kVisualFrameIndex)
        .setSemanticObjectMeasurementUncertainties(object_uncertainties);
    vertex.getVisualFrame(kVisualFrameIndex)
        .setSemanticObjectDescriptors(object_descriptors);
    vertex.getVisualFrame(kVisualFrameIndex)
        .setSemanticObjectClassIds(object_class_ids);
  }
}
