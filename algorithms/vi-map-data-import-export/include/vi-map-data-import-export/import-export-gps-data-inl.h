#ifndef VI_MAP_DATA_IMPORT_EXPORT_IMPORT_EXPORT_GPS_DATA_INL_H_
#define VI_MAP_DATA_IMPORT_EXPORT_IMPORT_EXPORT_GPS_DATA_INL_H_
...
void exportGpsDataMatchedToVerticesToCsv(
  ...
  std::vector<std::string> header_fields = {"# vertex timestamp [ns]",
                                            "vertex id"};
  addCsvHeaderFields<GpsMeasurement>(&header_fields);

  const std::string kDelimiter = ", ";
  file.writeDataWithDelimiterAndNewLine(kDelimiter, header_fields);
  ...
  for (const vi_map::MissionId& mission_id : mission_ids) {
    ...
    for (const pose_graph::VertexId& vertex_id : vertex_ids) {
      ...
      if (gps_utm_measurements.getNearestValueToTime(
        ...
        convertGpsMeasurementToCsvFields(gps_measurement, &csv_fields);
        file.writeDataWithDelimiterAndNewLine(kDelimiter, csv_fields);
      } else {
        LOG(WARNING)
...
#endif  // VI_MAP_DATA_IMPORT_EXPORT_IMPORT_EXPORT_GPS_DATA_INL_H_
