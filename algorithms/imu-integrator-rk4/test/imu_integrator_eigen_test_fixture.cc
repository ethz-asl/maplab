...
class ImuIntegratorEigenFixture : public ::testing::Test {
 protected:
  virtual void SetUp() {
  ...
  void constructIntegrator() {
    integrator_ = std::shared_ptr<ImuIntegratorEigen>(
        new ImuIntegratorEigen(
  ...
  std::shared_ptr<ImuIntegratorEigen> integrator_;
