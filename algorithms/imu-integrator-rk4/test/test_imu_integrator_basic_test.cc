...
TEST_F(PosegraphErrorTerms, ImuIntegratorZeroReadingsZeroBiases) {
...
TEST_F(PosegraphErrorTerms, ImuIntegratorJustNonzeroGravity) {
...
TEST_F(PosegraphErrorTerms, ImuIntegratorConstantForwardAcceleration) {
...
TEST_F(PosegraphErrorTerms, ImuIntegratorConstForwardAccelInitialVelocity) {
  gravity_magnitude_ = 9.81;
  constructIntegrator();
...
TEST_F(PosegraphErrorTerms, ImuIntegratorConstAccelAndVelocityWhenRotated) {
  gravity_magnitude_ = 9.81;
  constructIntegrator();
  ...
  EXPECT_NEAR_EIGEN(
      getOrientationFromState(next_state_),
      Eigen::Vector4d(0, 0, sqrt(2) / 2, sqrt(2) / 2), 1e-15);
  EXPECT_ZERO_EIGEN(getGyroBiasFromState(next_state_), 1e-15);
  EXPECT_NEAR_EIGEN(
      getVelocityFromState(next_state_),
      Eigen::Vector3d(0, expected_final_velocity, 0), 1e-15);
  EXPECT_ZERO_EIGEN(getAccelBiasFromState(next_state_), 1e-15);
  EXPECT_NEAR_EIGEN(
      getPositionFromState(next_state_),
...
TEST_F(PosegraphErrorTerms, ImuIntegratorConstantAngularVelocity) {
  gravity_magnitude_ = 9.81;
  constructIntegrator();
...
TEST_F(PosegraphErrorTerms, ImuIntegratorAngularVelocitySlope) {
  delta_time_seconds_ = 1.0;
  gravity_magnitude_ = 0.0;
  constructIntegrator();
...
TEST_F(PosegraphErrorTerms, ImuIntegratorLinearAccelerationSlope) {
