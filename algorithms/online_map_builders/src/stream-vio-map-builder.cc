...
DEFINE_bool(
    vio_map_builder_save_point_clouds_as_resources, false,
    "Store the point clouds associated with a lidar sensor to the map resource "
    "folder.");

namespace online_map_builders {

const vi_map::VIMap* StreamVioMapBuilder::constMap() const {
  return map_;
}

pose_graph::VertexId StreamVioMapBuilder::getRootVertexId() const {
  CHECK(mission_id_.isValid());
  return map_->getMission(mission_id_).getRootVertexId();
}

pose_graph::VertexId StreamVioMapBuilder::getLastVertexId() const {
  CHECK(last_vertex_.isValid());
  return last_vertex_;
}

StreamVioMapBuilder::StreamVioMapBuilder(
...
void StreamVioMapBuilder::apply(const vio::VioUpdate& update) {
  CHECK(update.check());
  constexpr bool kDeepCopyNFrame = true;
  apply(update, kDeepCopyNFrame);
}

void StreamVioMapBuilder::apply(
...
void StreamVioMapBuilder::addRootViwlsVertex(
...
void StreamVioMapBuilder::addViwlsVertexAndEdge(
    const aslam::VisualNFrame::Ptr& nframe,
...
pose_graph::VertexId StreamVioMapBuilder::addViwlsVertex(
    const aslam::VisualNFrame::Ptr& nframe,
    const vio::ViNodeState& vinode_state, const aslam::Transformation& T_G_M) {
  CHECK_EQ(
...
void StreamVioMapBuilder::removeAllVerticesAfterVertexId(
...
void StreamVioMapBuilder::addImuEdge(
