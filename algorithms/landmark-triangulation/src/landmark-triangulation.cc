...
void retriangulateLandmarksOfVertex(
  ...
  vi_map::LandmarkIdList landmark_ids_in_vertex;
  for (vi_map::Landmark& landmark : landmark_store) {
    landmark_ids_in_vertex.emplace_back(landmark.id());
  }
  retriangulateLandmarks(
      landmark_ids_in_vertex, storing_vertex.getMissionId(), map);
}
