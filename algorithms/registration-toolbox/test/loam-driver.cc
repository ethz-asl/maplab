...
static void registerCloud(
  ...
  auto aligner =
      regbox::BaseController::make("regbox::LoamController", "Loam");
  CHECK_NOTNULL(aligner);
  std::cout<<"starting alignment"<<std::endl;
  std::cout<<"target points: "<<target_cloud->points.size()<<std::endl;
  std::cout<<"source points: "<<source_cloud->points.size()<<std::endl;
