...
RegistrationResult PclAlignment<T_alignment, T_point>::registerCloudImpl(
  ...
  if (is_converged &&
      T.topLeftCorner<3, 3>().normalized().squaredNorm() <=
          1.0 + kindr::minimal::EPS<double>::normalization_value()) {
    ...
    is_converged &= aligner_.hasConverged();
    return RegistrationResult(
        transformed, cov, this->convertEigenToKindr(T), is_converged);
  } else {
    LOG(ERROR)
