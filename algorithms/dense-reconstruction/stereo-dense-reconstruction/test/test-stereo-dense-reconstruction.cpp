...
const std::string kTestDataBaseFolder =
    "./stereo_dense_reconstruction_test_data/";
 ...
 public:
  ...
  void loadSolution(const std::string& name) {
    img_disparity_solution_ = cv::imread(
        kTestDataBaseFolder + name + "_disparity_comparison.png",
        cv::IMREAD_UNCHANGED);
  ...
  void setupBikeStereoDataset() {
    ...
    cv::Mat img_left_color =
        cv::imread(kTestDataBaseFolder + "img_left.jpg", cv::IMREAD_COLOR);
    ...
    cv::Mat img_right_color =
        cv::imread(kTestDataBaseFolder + "img_right.jpg", cv::IMREAD_COLOR);
  ...
  void setupKittiStereoDataset() {
    ...
    img_left_ = cv::imread(
        kTestDataBaseFolder + "kitti_img_left.jpg", cv::IMREAD_UNCHANGED);
    CHECK(!img_left_.empty());
    CHECK_EQ(img_left_.type(), CV_8UC1);

    img_right_ = cv::imread(
        kTestDataBaseFolder + "kitti_img_right.jpg", cv::IMREAD_UNCHANGED);
    CHECK(!img_right_.empty());
    CHECK_EQ(img_right_.type(), CV_8UC1);
