#ifndef VI_MAP_SEMANTIC_LANDMARK_QUALITY_METRICS_INL_H_
#define VI_MAP_SEMANTIC_LANDMARK_QUALITY_METRICS_INL_H_
...
inline bool isSemanticLandmarkWellConstrained(
  ...
  if (signed_distance_to_closest_observer >
    ...
    statistics::StatsCollector stats("Semantic Landmark too close or too far away.");
  ...
  if (!quality_good) {
    statistics::StatsCollector stats(
        "Semantic Landmark observations angles too close together.");
    stats.IncrementOne();
  }
  return quality_good;
}
...
#endif  // VI_MAP_SEMANTIC_LANDMARK_QUALITY_METRICS_INL_H_
