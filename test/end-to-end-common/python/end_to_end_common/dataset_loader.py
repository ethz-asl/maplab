...
def load_dataset(csv_file, input_format="maplab_console"):
  ...
  Supported formats:
    ...
    - euroc_ground_truth (euroc ground truth CSVs as can be downloaded with the
                          datasets)
  ...
  if input_format.lower() == "maplab_console":
    ...
    estimator_data_raw = np.genfromtxt(csv_file, delimiter=',', skip_header=1)
    ...
    for i in range(num_elements):
      ...
      T_M_I = csv_row_data_to_transformation(estimator_data_raw[i, 8:11],
                                             estimator_data_raw[i, 11:15])
    ...
    trajectory = np.genfromtxt(csv_file, delimiter=',', skip_header=1)
    # We need to bring the Euroc ground truth data into the format required by
