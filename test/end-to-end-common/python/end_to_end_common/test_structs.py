...
class TestDataStruct:
  ...
  def __init__(self,
    ...
    for max_errors in max_errors_list:
      ...
      self.max_errors.orientation_mean_rad = self._get_min_positive_value(
          self.max_errors.orientation_mean_rad, max_errors.orientation_mean_rad)
      self.max_errors.orientation_rmse_rad = self._get_min_positive_value(
          self.max_errors.orientation_rmse_rad, max_errors.orientation_rmse_rad)
    ...
    if max_error > 0:
      if calculated_error > max_error:
        raise ThresholdExceededException(
          "The %s (%f) for the dataset %s exceeds the maximum allowed value " \
          "(%f)." % (text, calculated_error, self.label, max_error))
  ...
  def check_errors(self):
    ...
    if self.calculated_errors.position_mean_m >= 0 and \
        self.calculated_errors.position_rmse_m >= 0 and \
        self.calculated_errors.orientation_mean_rad >= 0 and \
        self.calculated_errors.orientation_rmse_rad >= 0:
      self._check_value("position mean", self.calculated_errors.position_mean_m,
                        self.max_errors.position_mean_m)
      self._check_value("position RMSE", self.calculated_errors.position_rmse_m,
