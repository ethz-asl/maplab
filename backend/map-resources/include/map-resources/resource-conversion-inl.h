...
void addColorToPointCloud(
  ...
  void addScalarToPointCloud(
    ...
    LOG(FATAL)
        << "This point cloud either does not support scalars/intensities "
        << "or it is not implemented!";
  }
  ...
  bool hasColorInformation(
      const resources::VoxbloxColorPointCloud& point_cloud);
  template <>
  ...
  bool convertDepthMapToPointCloud(
      ...
      const aslam::Camera& camera, PointCloudType* point_cloud) {
    CHECK_NOTNULL(point_cloud);
    ...
    resizePointCloud(
        point_index, has_color, kHasNormals, kHasScalar, kHasLabels,
        point_cloud);
  ...
  bool convertPointCloudToDepthMap(
    ...
    const bool has_color =
        hasColorInformation(point_cloud_C) && image != nullptr;
             ...
             &has_intensity, &has_color](const std::vector<size_t>& batch) {
              ...
              for (size_t idx : batch) {
                ...
                if (create_range_image) {
                  depth_value =
                      std::min(point_3d.norm(), max_valid_depth_value);
                } else {
                ...
                if (use_openni_format) {
                  depth_value =
                      std::min(depth_value * 1e3, max_valid_depth_value);
                }
    ...
    // the depth map.
    if (use_openni_format) {
      depth_function = [&depth_values, &depth_map](
