#ifndef MAPLAB_COMMON_UNIQUE_ID_H_
#define MAPLAB_COMMON_UNIQUE_ID_H_
...
namespace std {
inline ostream& operator<<(ostream& out, const common::Id& hash) {
  out << hash.printString();
  return out;
}

template <typename IdType>
inline ostream& operator<<(ostream& out, const common::UniqueId<IdType>& hash) {
  out << hash.printString();
  return out;
}

template <>
struct hash<common::Id> {
  std::size_t operator()(const common::Id& hashId) const {
    return std::hash<std::string>()(hashId.hexString());
  }
};
}  // namespace std

#endif  // MAPLAB_COMMON_UNIQUE_ID_H_
