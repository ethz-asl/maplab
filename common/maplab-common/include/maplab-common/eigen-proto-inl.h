...
inline void deserialize(
  ...
  *quaternion = map;
  CHECK_NEAR(1.0, quaternion->norm(), 1e-3);
}
...
inline void deserialize(
  ...
  CHECK_NEAR(
      1.0, transformation->getRotation().toImplementation().norm(), 1e-3);
}
...
inline void serialize(
    ...
    google::protobuf::RepeatedField<Scalar>* proto) {
  CHECK_NOTNULL(proto);
  CHECK_NEAR(1.0, transformation.getRotation().toImplementation().norm(), 1e-3);
