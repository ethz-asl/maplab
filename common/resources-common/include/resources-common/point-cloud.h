...
struct PointCloud {
  ...
  inline void append(const PointCloud& other) {
    ...
    times_ns.insert(
        times_ns.end(), other.times_ns.begin(), other.times_ns.end());
  ...
  inline void appendTransformed(
    ...
    // Remember how many points there were before.
  ...
  inline bool loadFromFile(const std::string& file_path) {
    ...
    if (stream_ply.is_open()) {
      ...
      const int scalars_count = ply_file.request_properties_from_element(
